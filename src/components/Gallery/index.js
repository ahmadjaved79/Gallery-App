...
class Gallery extends Component {
  ...
  onChangeGallery = (imgUrl, imgAltText) => {
    this.setState({imageUrl: imgUrl, imageAltText: imgAltText})
  }
  render() {
    const {imageUrl, imageAltText} = this.state
    return (
      <div className="gallery-container">
        <img src={imageUrl} alt={imageAltText} className="gallery-image" />
        <h1 className="title">Nature Photography</h1>
        <p className="description">Nature Photography by Rahul</p>
        <ul className="tumbnail-container">
          {imagesList.map(eachImage => (
            <ThumbnailItem
              key={eachImage.id}
              imageDetails={eachImage}
              onChangeGallery={this.onChangeGallery}
